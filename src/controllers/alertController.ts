...
export const createAlerta = async (req: Request, res: Response) => {            //  Controlador para crear una nueva alerta
    try {
        ...
        const newAlerta = await Alerta.create({                   // Creamos una nueva alerta en la base de datos usando Sequelize
            titulo,        
            descripcion,   
            categoria,
            ubicacion,      
        ...
        res.status(201).json({ message: 'Alerta creada con éxito', alert: newAlerta })           // Respondemos con código 201 (creado exitosamente) y la alerta creada
    } catch (error) {
...
export const listAlerta = async (_req: Request, res: Response) => {               //  Controlador para listar todas las alertas registradas
    try {
